...
(define-type Stack-dec
  [empS]
  [numS [n : Exp] [s : Stack-dec]]
  [opS  [o : Exp] [s : Stack-dec]])

(define (push-s [x : Exp] [s : Stack-dec]) : Stack-dec
  (type-case Exp x
    [(numE n)
     (numS (numE n) s)]
    [(opE o l r)
...
(define (pop-s [s : Stack-dec]) : (Exp * Stack-dec)
  (type-case Stack-dec s
    [(empS)
     (error 'pop-s "stack is empty!")]
    [(numS n s)
     (pair n s)]
    [(opS o s)
     (pair o s)]))

(define (decompile [e : (Listof Instr)]) : Exp
  (local [(define (tmp-dec [e : (Listof Instr)] [s : Stack-dec])
            (type-case (Listof Instr) e
              [empty
               (fst (pop-s s))]
              [(cons x e)
               (type-case Instr x
                 [(pushI n)
                  (tmp-dec e (push-s (numE n) s))]
                 [(opI o)
                  (let* ([n2-s2 (pop-s s)]
                         [n2 (fst n2-s2)]
                         [s2 (snd n2-s2)]
